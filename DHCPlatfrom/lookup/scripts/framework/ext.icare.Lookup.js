...
dhcc.icare.lookupconfig = {
	...
	hide: function(lookupName){
	    dhcc.icare.lookupconfig.lookupDiv.style.display = "none"; //add 2012/07/01	    
	   	var prelu = dhcc.icare.lookupconfig.preLookup;
		if(prelu && prelu.store && prelu.lookupName!=lookupName) {	
			//2015-04-18 上一个lookup不是当前这个
			dhcc.icare.lookupconfig.preLookup.destroy();
...
dhcc.icare.Lookup = Ext.extend(Ext.grid.GridPanel, {  
    ...
    */
    ...
    initComponent: function() {
    	...
    	dhcc.icare.lookupconfig.hide(this);
	...
	initGridPanel: function (){
		...
		var pagingBar = new Ext.PagingToolbar({
			...
			items: ['-',new Ext.Toolbar.Button({handler: dhcc.icare.lookupconfig.hide,text:'关闭',scope:this})]
    	});
    	var cms = json.cms;
	...
	posiAndSizeResize: function(){		
		...
		dhcc.icare.lookupconfig.lookupDiv.style.top = tmpTop ;  //pixelTop
		dhcc.icare.lookupconfig.lookupDiv.style.left = tmpLeft ;  //pixelLeft
	...
	headerDblClick: function(){
		if(this.lookupListComponetId != ""){
			...
			if(flag==1) websys_lu('../csp/websys.component.customiselayout.csp?ID='+this.lookupListComponetId+'&CONTEXT=K'+this.listClassName+':'+this.listQueryName+"&DHCICARE=1",false);		
		}
	},
    rowClick: function(gridPanel, rowIndex, e) {
	    ...
	    //dhcc.icare.lookupconfig.hide(this.lookupName);
		setTimeout(dhcc.icare.lookupconfig.hide("force"),1000); //this.lookupName  
    },
    getValue: function() {
	    this.value;       
    },
